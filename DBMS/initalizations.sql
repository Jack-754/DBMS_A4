...
CREATE TABLE IF NOT EXISTS village (
    ...
    pradhan_id INT UNIQUE, -- Ensures one Pradhan per village
    FOREIGN KEY (pradhan_id) REFERENCES citizens(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS citizens (
   ...
   CONSTRAINT head_must_be_in_same_household CHECK (
       (household_id = (SELECT household_id FROM households WHERE head_of_household = citizens.id))
   )
);
